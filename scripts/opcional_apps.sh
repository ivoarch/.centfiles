...
# Administration
yum install -y gnome-system-monitor gnome-system-log htop ps_mem seahorse seahorse-plugins keepassx system-config-language keychecker rpmreaper yum-utils beesu policycoreutils-gui system-config-firewall inotify-tools rpmdevtools rpm-build redhat-rpm-config ntsysv rpmconf rpmrebuild fslint
...
yum install -y nmap nmap-frontend dsniff ettercap tcpdump sslstrip wireshark-gnome nload arp-scan
